...
public:
    ...
    bool DrawModelIfLoaded(const std::string& path, const VECTOR& pos, float rotY, float scale, float rotX = 0.0f, float rotZ = 0.0f);

    // ------------------------------------------------------------------------
    // プロシージャル（3Dプリミティブ）フォールバック描画関数群
    // ------------------------------------------------------------------------
    // プレイヤー（猫）のフォールバック描画
    void DrawFallbackCat(const VECTOR& pos, float rotY, int repCount, bool isSoreness, bool isPouncing, float animTime);
    ...
    // 障害物のフォールバック描画
    void DrawFallbackObstacle(const VECTOR& pos, float width, float height, float depth, unsigned int mainColor, unsigned int frameColor, const std::string& name);
